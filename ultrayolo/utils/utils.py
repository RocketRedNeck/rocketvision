...
def plot_reticle(x, img, color=None, label=None, line_thickness=None, scale = 1, timestamp = None):
    ...
    if label:
        ...
        cv2.putText(img, label, ((c1[0]+c2[0])//2 - 5,(c1[1]+c2[1])//2 + 15), 0, tl / 3, color, thickness=tf, lineType=cv2.LINE_AA)
...
def plot_one_tag(xyxy, img, color=None, label=None, line_thickness=None, scale = 1):
    ...
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = mx + t_size[0], my - t_size[1] - 3
        cv2.rectangle(img, (mx,my), c2, color, -1)  # filled
        cv2.putText(img, label, (mx, my - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
